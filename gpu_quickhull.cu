...
struct MinXOp {
  __host__ __device__ IndexedPoint operator()(const IndexedPoint &a,
...
static __global__ void buildPointArray(const float *px, const float *py,
                                       IndexedPoint *out, int n) {
...
static void findMinMaxX_CUB(const float *d_px, const float *d_py, int n,
                            IndexedPoint &h_min, IndexedPoint &h_max) {
  ...
  cub::DeviceReduce::Reduce(d_temp, temp_bytes, d_points, d_min, n, MinXOp(),
  ...
  cub::DeviceReduce::Reduce(d_temp, temp_bytes, d_points, d_max, n, MaxXOp(),
                            IndexedPoint{-FLT_MAX, -FLT_MAX, -1});
...
struct MaxDistOp {
  __host__ __device__ DistIdxPair operator()(const DistIdxPair &a,
                                             const DistIdxPair &b) const {
    return (a.dist >= b.dist) ? a : b;
  }
};

__global__ void buildDistIdxArray(const float *distances, DistIdxPair *pairs,
                                  int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
...
__global__ void findSegmentOffsetsKernel(const int *labels, int *offsets,
                                         int numSegments, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
...
void segmentedMaxDistReduce(const float *d_distances, const int *d_labels,
                            int *d_segmentOffsets, DistIdxPair *d_maxPerSegment,
  ...
  cub::DeviceSegmentedReduce::Reduce(
      d_temp, temp_bytes, d_pairs, d_maxPerSegment, numSegments,
      d_segmentOffsets, d_segmentOffsets + 1, MaxDistOp(), identity);
...
__global__ void computeDistancesKernel(float *px, float *py, int *labels,
  ...
  int ansNum = maxLabel - minLabel + 2;
  for (int i = tid; i < ansNum; i += blockDim.x) {
...
static void cubExclusiveScanInt(int *d_input, int *d_output, int n) {
  ...
  cub::DeviceScan::ExclusiveSum(d_temp_storage, temp_storage_bytes, d_input,
                                d_output, n);
...
__global__ void determineSide(float *px, float *py, int *labels, float *ansX,
                              float *ansY, int *state,
                              DistIdxPair *maxIdxPerLabel, int *goesLeft,
                              int *goesRight, int n) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= n)
    return;
...
__global__ void buildNewAnsKernel(float *oldAnsX, float *oldAnsY,
                                  float *newAnsX, float *newAnsY, float *px,
                                  float *py, int *state, int *statePrefixSum,
                                  DistIdxPair *maxPerSegment, int numLabels) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i > numLabels)
    return;
...
static void gpuQuickHullOneSide(float *h_px, float *h_py, int n, float leftX,
                                float leftY, float rightX, float rightY,
  ...
  while (true) {
    ...
    // Shared memory needs to hold up to (numLabels + 2) entries for both X and
    // Y
    ...
    determineSide<<<numBlocks, BLOCK_SIZE>>>(d_px, d_py, d_labels, d_ansX,
                                             d_ansY, d_state, d_maxPerSegment,
                                             d_goesLeft, d_goesRight, currentN);
    ...
    cudaMemcpy(&lastState, d_state + numLabels - 1, sizeof(int),
               cudaMemcpyDeviceToHost);
    cudaMemcpy(&lastStatePrefixSum, d_statePrefixSum + numLabels - 1,
    ...
    computeNewLabelsKernel<<<numBlocks, BLOCK_SIZE>>>(
        d_labels, d_statePrefixSum, d_goesLeft, d_goesRight, d_newLabels,
        d_keepFlags, currentN);

    int *d_labelCounts, *d_labelOffsets, *d_labelCounters;
    cudaMalloc(&d_labelCounts, newNumLabels * sizeof(int));
    cudaMalloc(&d_labelOffsets, (newNumLabels + 1) * sizeof(int));
    ...
    int newN;
    cudaMemcpy(&newN, d_labelOffsets + newNumLabels, sizeof(int),
               cudaMemcpyDeviceToHost);
    ...
    buildNewAnsKernel<<<ansBlocks, BLOCK_SIZE>>>(
        d_ansX, d_ansY, d_newAnsX, d_newAnsY, d_px, d_py, d_state,
        d_statePrefixSum, d_maxPerSegment, numLabels);

    cudaMemcpy(d_ansX, d_newAnsX, newAnsSize * sizeof(float),
               cudaMemcpyDeviceToDevice);
    cudaMemcpy(d_ansY, d_newAnsY, newAnsSize * sizeof(float),
               cudaMemcpyDeviceToDevice);
    cudaFree(d_newAnsX);
    cudaFree(d_newAnsY);
    ...
    computeLocalScatterIdx<<<numBlocks, BLOCK_SIZE>>>(
        d_newLabels, d_keepFlags, d_labelOffsets, d_scatterIdx, d_labelCounters,
        currentN);
    ...
    compactPointsKernel<<<numBlocks, BLOCK_SIZE>>>(
        d_px, d_py, d_newLabels, d_px_new, d_py_new, d_labels_new, d_keepFlags,
        d_scatterIdx, currentN);
...
extern "C" void gpuQuickHull(float *h_px, float *h_py, int n, float *result_x,
                             float *result_y, int *M) {
  if (n <= 2) {
    for (int i = 0; i < n; i++) {
      result_x[i] = h_px[i];
      result_y[i] = h_py[i];
  ...
  if (!upperX.empty()) {
    gpuQuickHullOneSide(upperX.data(), upperY.data(), upperX.size(), minPt.x,
                        minPt.y, maxPt.x, maxPt.y, upperHull);
  }

  std::vector<Point> lowerHull;
  if (!lowerX.empty()) {
    gpuQuickHullOneSide(lowerX.data(), lowerY.data(), lowerX.size(), maxPt.x,
                        maxPt.y, minPt.x, minPt.y, lowerHull);
  }
