...
__global__ void computeDistancesKernel(float *px, float *py, float lx, float ly,
                                       float rx, float ry, float *distances,
                                       int n) {
...
__global__ void compactPointsKernel(float *px, float *py, float *newDistLeft,
                                    float *newDistRight, int *goesLeft,
                                    int *goesRight, int *leftScan,
                                    int *rightScan, int rightOffset,
                                    float *pxNew, float *pyNew, float *distNew,
                                    int n) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= n)
    return;
  ...
  while (true) {
    ...
    for (size_t p = 0; p < partitions.size(); p++) {
      ...
      computeDistancesKernel<<<numBlocks, BLOCK_SIZE>>>(
          d_px + part.start, d_py + part.start, L.x, L.y, R.x, R.y,
      ...
      cub::DeviceReduce::ArgMax(d_temp, temp_bytes, d_distances + part.start,
                                d_maxValue, d_maxIdx, part.count);
      ...
      float maxPx, maxPy;
      cudaMemcpy(&maxPx, d_px + part.start + h_maxIdx, sizeof(float),
                 cudaMemcpyDeviceToHost);
      cudaMemcpy(&maxPy, d_py + part.start + h_maxIdx, sizeof(float),
                 cudaMemcpyDeviceToHost);

      newAns.push_back({maxPx, maxPy});

      classifyPointsKernel<<<numBlocks, BLOCK_SIZE>>>(
          d_px + part.start, d_py + part.start, d_distances + part.start, L.x,
          L.y, maxPx, maxPy, R.x, R.y, h_maxIdx, d_goesLeft + part.start,
          d_goesRight + part.start, d_newDistLeft + part.start,
          d_newDistRight + part.start, part.count);
      cudaDeviceSynchronize();

      cubExclusiveScanInt(d_goesLeft + part.start, d_leftScan + part.start,
                          part.count);
      cubExclusiveScanInt(d_goesRight + part.start, d_rightScan + part.start,
                          part.count);

      int leftCount, rightCount;
      int lastLeft, lastRight;
      cudaMemcpy(&leftCount, d_leftScan + part.start + part.count - 1,
                 sizeof(int), cudaMemcpyDeviceToHost);
      cudaMemcpy(&lastLeft, d_goesLeft + part.start + part.count - 1,
                 sizeof(int), cudaMemcpyDeviceToHost);
      leftCount += lastLeft;
      cudaMemcpy(&rightCount, d_rightScan + part.start + part.count - 1,
                 sizeof(int), cudaMemcpyDeviceToHost);
      cudaMemcpy(&lastRight, d_goesRight + part.start + part.count - 1,
                 sizeof(int), cudaMemcpyDeviceToHost);
      ...
      compactPointsKernel<<<numBlocks, BLOCK_SIZE>>>(
          d_px + part.start, d_py + part.start, d_newDistLeft + part.start,
          d_newDistRight + part.start, d_goesLeft + part.start,
          d_goesRight + part.start, d_leftScan + part.start,
          d_rightScan + part.start, leftCount, d_pxNew, d_pyNew, d_distNew,
          part.count);
      cudaDeviceSynchronize();

      cudaMemcpy(h_pxTemp, d_pxNew, (leftCount + rightCount) * sizeof(float),
                 cudaMemcpyDeviceToHost);
      cudaMemcpy(h_pyTemp, d_pyNew, (leftCount + rightCount) * sizeof(float),
                 cudaMemcpyDeviceToHost);
    ...
    if (currentN > 0) {
      cudaMemcpy(d_px, allNewPx.data(), currentN * sizeof(float),
                 cudaMemcpyHostToDevice);
      cudaMemcpy(d_py, allNewPy.data(), currentN * sizeof(float),
                 cudaMemcpyHostToDevice);
    }
  }
